...
export const Scenario = types
  .model('Scenario', {
    ...
    const createTerminal = flow(function* () {
      const rawResponse = yield fetch('http://api.v1.kfcoding.com/api/basic/workspaces', {
        method: 'POST',
        headers: {
          'Accept': 'application/json',
          'Content-Type': 'application/json'
        },
        body: JSON.stringify({dockerImage: 'cpp', type: 'terminal'})
      });
    ...
    return {
      ...
      isVisited() {
        ...
        let visited = false;
        for (let i in vss) {
          if (vss[i] == getRoot(self).repo + self.dir) {
            visited = true;
            break
          }
        }
